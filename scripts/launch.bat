...
docker stop ecu_monitor_live_legacy >nul 2>&1
docker rm ecu_monitor_live_legacy >nul 2>&1

echo [2/3] Building and Starting ECU System...
:: Now it will find the Dockerfile correctly
docker build -t ecu-monitor-system .
docker run -d -p 8000:8000 --name ecu_monitor_live_legacy ecu-monitor-system
